...
### Step debug smoke test



Use the step-by-step debug mode when the normal hotkey is too fast to inspect visually or when the target menu position needs manual confirmation.
...
2. Select a short text fragment.
3. Press `Ctrl+F12`.
4. Confirm the first dialog to open the browser context menu.
5. Confirm the second dialog to send the configured number of `{Down}` presses.
6. Confirm the third dialog to press `Enter` on the current menu item.
7. Press `Cancel` in any dialog and verify the flow stops safely without being treated as a runtime failure.
...
The script writes runtime diagnostics to `logs/ahk-runtime.log` to make hotkey behavior and Alice flow steps easier to inspect during manual validation.
...
- `Hotkey pressed: F8`, `Hotkey pressed: F9`, `Hotkey pressed: F10`, and `Hotkey pressed: Ctrl+F12` entries for supported hotkeys.
- `FLOW_START` and `FLOW_DONE` entries for the main Alice flow.
- `STEP_START` and `STEP_DONE` entries for opening the context menu, moving to the configured menu item, and confirming the final selection.
- `FLOW_CANCELLED` when the step debug flow is stopped intentionally with `Cancel`.
- `FLOW_FAIL` if an actual runtime error occurs.



When debugging the primary flow, `Ctrl+F12` can be used as a slower confirmation-based mode that pauses before each major step without triggering the browser's default DevTools shortcut on `F12`.
...
The current main script is focused on a primary Alice-related flow plus basic debug helpers:



- `F8` shows a short help notification.
- `F9` triggers reading of selected text through the Alice-related context menu.
- `F10` triggers the alternate configured menu item.
- `Ctrl+F12` runs the primary flow in step debug mode with explicit confirmation before each major action.
